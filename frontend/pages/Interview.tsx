...
export const Interview: React.FC<{ id: string }> = ({ id }) => {
  ...
  useEffect(() => {
    const fetchSession = async () => {
      ...
      if (currentSession.turns.length > 0 && currentSession.status === 'planned') {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setSession(prev => prev ? { ...prev, turns: [...prev.turns, aiTurn] } : null);
  ...
  const handleHint = async () => {
    ...
    try {
      const lastAIQuestion = session.turns.filter(t => t.role === 'ai').slice(-1)[0]?.text;
      const conversationContext = session.turns.map(t => `${t.role}: ${t.text}`).join('\n');
      const hint = await geminiService.getHint(lastAIQuestion, conversationContext);
      ...
      setSession(prev => prev ? { ...prev, turns: [...prev.turns, hintTurn] } : null);
      ...
      <div className="bg-slate-900 text-white px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400">{session.companyName} Pack Loaded</p>
          </div>
        </div>
        <div className="flex items-center gap-4">
          ...
          >
            {hintLoading ? <i className="fa-solid fa-circle-notch fa-spin"></i> : <i className="fa-solid fa-lightbulb"></i>}
          </button>
          <button 
      ...
      <div className="p-4 border-t border-slate-100 bg-white">
        <div className="flex gap-2 max-w-3xl mx-auto items-end">
          ...
          <textarea 
            ...
            className="flex-1 p-3 border border-slate-200 rounded-xl outline-none resize-none h-12" 
            rows={1} 
          />
          <button 
